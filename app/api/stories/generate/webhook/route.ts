...
//     const { storyId, text, voiceId, language = 'english' } = body;
...
//     // Use provided text and voiceId or fall back to story data
//     const textToProcess = text || story.text_content;
//     const voiceToUse = voiceId || story.voice || 'default';
//     const languageToUse = language || story.language || 'english';
    
//     console.log(`[WEBHOOK] Processing story ${storyId} with voice ${voiceToUse} in ${languageToUse}`);
...
//       audioContent = await generateAudioWithElevenLabs(textToProcess, voiceToUse, languageToUse);
...
//       console.timeEnd('elevenlabs-tts');
//     } catch (audioError) {
//       console.error('[WEBHOOK] Error generating audio:', audioError);
//       return NextResponse.json({ 
//         success: false, 
...
//       const uploadResult = await cloudinary.uploader.upload(audioContent, {
//         resource_type: 'auto',
//         folder: 'story-app-audio',
//         public_id: `${storyId}-audio`,
//         overwrite: true
...
//       console.log(`[WEBHOOK] Audio uploaded to Cloudinary: ${audioUrl}`);
      
//       // Mix with background music if available
//       if (story.background_music_id && await isSubscriber(session.user.id)) {
//         console.log(`[WEBHOOK] Mixing audio with background music: ${story.background_music_id}`);
//         console.time('audio-mixing');
//         try {
//           // Mix audio with background music
//           const mixResult = await mixAudioWithBackgroundMusic(
//             audioUrl,
//             story.background_music_id,
//             0.25 // Setting background music at 25% volume
//           );
...
//       }
//     } catch (uploadError) {
//       console.error('[WEBHOOK] Error uploading audio to Cloudinary:', uploadError);
//       return NextResponse.json({ 
//         success: false, 
...
//       if (updateError) {
//         console.error('[WEBHOOK] Error updating story with audio URL:', updateError);
//         throw new Error('Failed to update story with audio URL');
//       }
//     } catch (updateError) {
//       console.error('[WEBHOOK] Error updating story in database:', updateError);
//       // Even if the database update fails, return the audio URL to the client
//     }
    
//     // Return success response
//     console.log('[WEBHOOK] Audio generation and upload successful');
//     return NextResponse.json({
//       success: true,
//       storyId,
//       audioUrl,
//       hasMixedAudio: story.background_music_id !== null
//     });
