// app/dashboard/stories/[storyId]/story-viewer.tsx
"use client";
...
export default function StoryViewer({ initialStory }: StoryViewerProps) {
  ...
  return (
    <div className="container max-w-4xl mx-auto px-4 py-8 animate-in fade-in-50 duration-500">
      ...
      {/* Story Text - Now with pagination indicator if it's a long story */}
      <Card className="bg-gray-900 border-gray-800 p-6 animate-in fade-in slide-in-from-bottom-5 duration-500 delay-300">
        {isLongStory && textLoaded && (
          <div className="flex items-center justify-between mb-4">
            <div className="text-sm text-gray-400">
              <BookOpen className="h-4 w-4 inline mr-2" />
              This is a longer story that's better read in paginated view
            </div>
            <Dialog>
              <DialogTrigger asChild>
                <Button 
                  variant="outline" 
                  size="sm"
                  className="text-indigo-300 border-indigo-800 bg-indigo-950/50 hover:bg-indigo-900/30"
                >
              ...
              <DialogContent className="sm:max-w-6xl p-0 bg-transparent border-0 h-[90vh] overflow-y-auto">
                {storyPlayerData && (
                  <StoryPlayer 
                    story={storyPlayerData} 
                    onClose={() => document.getElementById('close-dialog-button-2')?.click()}
                    className="min-h-fit"
                  />
                )}
                <button id="close-dialog-button-2" className="hidden" />
              </DialogContent>
            </Dialog>
          </div>
        )}
        
        {/* Scrollable container for very long stories */}
        <ScrollArea className={isLongStory && textLoaded ? "max-h-[600px]" : ""}>
